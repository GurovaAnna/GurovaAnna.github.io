...
var skillsList=[];
skillsList = _.uniq(_.flatten(_.map(data, 'skills')));
skillsList = _.sortBy(skillsList, function(str){
...
var friendsCount = [];
var nameList = [];
var nameObj = {}; //вспомагательный объект
_.forIn(data, function(value, key) {
    friendsCount[key] = value.friends.length;
    listOfFriends[key] = value.friends;
});
nameList = _.map(data, 'name');
nameObj = _.zip(nameList, friendsCount);
nameList = _.unzip(_.sortBy(nameObj, function (arr) {
      return arr[1];
}))[0];
listOfFriends = _.flatten(listOfFriends);
listOfFriends = _.uniq(_.map(listOfFriends, 'name'));
console.log(nameList);
console.log(_.sortBy(friendsCount));
